...
class Tools_Db_Repair_Mysql4
{
    ...
    public function setConnection(array $config, $type)
    {
        ...
        if (!$connection = @mysqli_connect($config['hostname'], $config['username'], $config['password'], $config['database'])) {
            throw new Exception(sprintf('%s database connection error: %s', ucfirst($type), mysqli_connect_error()));
        }
        /*if (!@mysqli_select_db($connection, $config['database'])) {
            throw new Exception(sprintf('Cannot select %s database (%s): %s', $type, $config['database'], mysqli_connect_error()));
        }*/
...
<div class="footer-container">
    <div class="footer">
        <p class="legality">Magento is a trademark of Irubin Consulting Inc. DBA Varien. Copyright Â© {$date} Irubin Consulting Inc.</p>
    </div>
</div>
</body>
</html>
